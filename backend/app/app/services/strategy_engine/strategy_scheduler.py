...
class StrategySchedulerService:
    ...
    def _get_strategy_market_config(self, instance: Any) -> Dict[str, Any]:
        ...
        if not symbol:
            raise ValueError(
                f"Strategy definition {definition.id} must specify a symbol in config_json"
            )
    ...
    def _resolve_market_data(self, instance: Any, config: Dict[str, Any]) -> Dict[str, Any]:
        """Fetch a current authenticated broker quote and normalize it for StrategyRunner."""
        if not self.broker_service:
            raise RuntimeError("BrokerService is required for automated strategy market data.")
        ...
        # Feed the same quote through the existing MarketDataProvider/EventBus path.
        # The provider performs timestamp/price validation and publishes quote.updated.
        if self.market_data_provider:
            event = self.market_data_provider.process_and_publish_quote(
                user_id=instance.user_id,
                broker_id=instance.broker_id,
                raw_quote=market_data,
            )
            if event and event.event_type.value == "quote.stale":
                raise RuntimeError(
                    f"Market quote rejected by MarketDataProvider for {exchange}:{symbol}"
                )
            if event:
                market_data.update(event.payload)
