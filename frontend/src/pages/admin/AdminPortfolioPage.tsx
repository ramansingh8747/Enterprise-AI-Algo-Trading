...
const AdminPortfolioPage: React.FC = () => {
  ...
  const handleDelete = async (item: AdminPortfolioItem) => {
    ...
    setDeletingId(item.portfolio_ref);
    try {
  ...
  return <div className="admin-control-center">
    ...
    <main className="admin-control-center__content"><header className="admin-control-center__header"><div><p className="admin-control-center__eyebrow">ADMIN CONTROL CENTER</p><h1>Portfolio</h1><p className="admin-control-center__subtitle">Persisted PAPER portfolios and application-owned LIVE accounts with valuation and P&amp;L.</p></div><div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}><button type="button" onClick={()=>void load(page,true)} disabled={refreshing} className="admin-control-center__refresh">{refreshing?'Refreshing…':'Refresh'}</button><button type="button" onClick={handlePurgeTests} disabled={isPurging} style={{ padding: '0.45rem 0.85rem', background: isPurging ? '#6b7280' : '#dc2626', color: '#ffffff', border: 'none', borderRadius: '0.375rem', fontWeight: 600, cursor: isPurging ? 'not-allowed' : 'pointer', fontSize: '0.8rem' }}>{isPurging ? 'Cleaning…' : 'Clean Sample Portfolios'}</button></div></header>
      ...
      <section className="admin-control-center__panel"><div className="admin-control-center__panel-header"><div><h2>Portfolio Accounts</h2><p>PostgreSQL is the source of truth. LIVE cash is not fabricated where no persisted broker cash snapshot exists.</p></div><span className={`admin-portfolio__ws ${websocketState==='CONNECTED'?'is-good':''}`}>● {websocketState}</span></div>
        ...
        {loading?<div className="admin-control-center__loading">Loading persisted portfolios…</div>:items.length===0?<div className="admin-control-center__empty">No portfolio accounts match the selected filters.</div>:<div className="admin-portfolio__table-wrap"><table className="admin-portfolio__table"><thead><tr><th>User</th><th>Mode</th><th>Broker</th><th>Strategy</th><th>Cash</th><th>Invested</th><th>Market Value</th><th>Realized P&amp;L</th><th>Unrealized P&amp;L</th><th>Equity</th><th>Positions</th><th>Updated</th><th className="admin-portfolio__th-actions">Actions</th></tr></thead><tbody>{items.map(item=><tr key={`${item.source}:${item.portfolio_ref}`}><td><strong>{item.user_name}</strong><small>{item.user_role}</small></td><td><span className={`admin-portfolio__mode ${item.execution_mode==='LIVE'?'is-live':'is-paper'}`}>{item.execution_mode}</span></td><td>{item.broker_name||'—'}</td><td>{item.strategy_name||'Manual / Default'}</td><td>{money(item.cash_balance)}</td><td>{money(item.invested_value)}</td><td>{money(item.market_value)}</td><td className={Number(item.realized_pnl)>=0?'admin-portfolio__profit':'admin-portfolio__loss'}>{money(item.realized_pnl)}</td><td className={Number(item.unrealized_pnl)>=0?'admin-portfolio__profit':'admin-portfolio__loss'}>{money(item.unrealized_pnl)}</td><td>{money(item.equity)}</td><td>{item.position_count}</td><td>{date(item.updated_at)}</td><td className="admin-portfolio__td-actions"><div style={{ display: "flex", gap: "0.4rem", justifyContent: "center" }}><button className="admin-portfolio__details" type="button" onClick={()=>setSelected(item)}>View</button>{item.source === "PAPER_PORTFOLIO" && <button type="button" className="admin-portfolio__delete-btn" disabled={deletingId === item.portfolio_ref} onClick={()=>handleDelete(item)}>{deletingId === item.portfolio_ref ? '…' : 'Delete'}</button>}</div></td></tr>)}</tbody></table></div>}
